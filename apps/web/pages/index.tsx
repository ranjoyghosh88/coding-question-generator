...
export default function Home(){
  ...
  async function generate(){
    setLoading(true);
    try {
      const r = await fetch(`${API}/generate`, { method: 'POST', headers: { 'Content-Type':'application/json' }, body: JSON.stringify({ language }) });
  ...
  async function runSolution(sol: Solution){
    ...
    try {
      const r = await fetch(`${API}/run`, { method: 'POST', headers: { 'Content-Type':'application/json' }, body: JSON.stringify({
        code: sol.code, language, runnerInput: sol.runnerInput
      })});
  ...
  return (
    <main style={{maxWidth: 1200, margin:'24px auto', fontFamily:'Inter, system-ui, sans-serif'}}>
      ...
      <p>Pick a language, generate a question, see multiple solutions and run JavaScript or TypeScript answers.</p>

      <div style={{display:'flex', gap:12, alignItems:'center', margin:'12px 0'}}>
        <label>Language: <select value={language} onChange={e => setLanguage(e.target.value as Lang)}>
